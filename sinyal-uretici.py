...
def main():
    ...
    # Her sembol için lokal durumları takip et
    ...
    while True:
        try:
            # Önce aktif tradeleri veritabanından çek ve kontrol et
            ...
            # Sinyal aranacak sembolleri belirle (aktif işlemi olmayanlar)
                ...
                if 4 <= current_ny_time.hour:
                    for symbol in symbols_to_scan:
                        ...
                        if new_signal:
                            ...
                            logging.info(f"[{symbol}] YENİ SİNYAL BULUNDU ve veritabanına eklendi: {signal_data}")

            time.sleep(config['loop_intervals']['signal_generator'])
        except Exception as e:
            logging.critical(f"Ana döngüde kritik hata: {e}")
            time.sleep(60)
